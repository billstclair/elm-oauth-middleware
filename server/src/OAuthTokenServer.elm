...
import OAuth exposing (HttpError(..))
...
type Msg
    ...
    | ReceiveToken Server.Http.Id String (List String) (Maybe String) (Result HttpError ResponseToken)
...
authenticationErrorToString : HttpError -> String
...
httpErrorToString error =
    case error of
        ...
        BadStatus code _ ->
            "BadStatus, code: " ++ String.fromInt code
...
-}
adjustRequest : (Result HttpError ResponseToken -> Msg) -> RequestParts Msg -> RequestParts Msg
    ...
    let
        ...
        expect =
            Http.expectStringResponse tagger
                (AC.authenticationResponseHandler ResponseToken.responseTokenDecoder)
    in
    { method = req.method
    , headers = headers
    , url = req.url
...
tokenRequest : (Result HttpError ResponseToken -> Msg) -> RedirectState -> String -> Model -> Result String (RequestParts Msg)
